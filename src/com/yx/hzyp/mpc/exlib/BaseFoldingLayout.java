...
public class BaseFoldingLayout extends ViewGroup {
	...
	/**
	 * The custom exception to be thrown so as to limit the number of views in this layout to at most one.
	 */
	...
	/**
	 * This method is called in order to update the fold's orientation, anchor point and number of folds. This creates the necessary setup in order to prepare the layout for a fold with the specified parameters. Some of the dimensions required for the folding transformation are
	 * also acquired here.
	 * 
	...
	private void prepareFold(Orientation orientation, float anchorFactor, int numberOfFolds) {
		...
		/*
		 * Loops through the number of folds and segments the full layout into a number of smaller equal components. If the number of folds is odd, then one of the components will be smaller than all the rest. Note that deltap below handles the calculation for an odd number of
		 * folds.
		 */
		for (int x = 0; x < mNumberOfFolds; x++) {
			if (mIsHorizontal) {
	...
	private void calculateMatrices() {
		...
		/**
		 * If the fold factor is 1 than the folding view should not be seen and the canvas can be left completely empty.
		 */
		if (mFoldFactor == 1) {
			mShouldDraw = false;
			return;
		...
		if (mFoldFactor > 0.9f && mFoldFactor > mPreviousFoldFactor && mFoldListener != null) {

			mFoldListener.onEndFold(mFoldFactor);
		} else if (mFoldFactor < 0.1f && mFoldFactor < mPreviousFoldFactor && mFoldListener != null) {
		...
		/*
		 * For an odd number of folds, the rounding error may cause the translatedDistancePerFold to be grater than the max fold width or height.
		 */
		...
		/*
		 * The size of some object is always inversely proportional to the distance it is away from the viewpoint. The constant can be varied to to affect the amount of perspective.
		 */
		...
		for (int x = 0; x < mNumberOfFolds; x++) {
			...
			/*
			 * Pixel fractions are present for odd number of folds which need to be rounded off here.
			 */
			for (int y = 0; y < 8; y++) {
			...
			/*
			 * If it so happens that any of the folds have reached a point where the width or height of that fold is 0, then nothing needs to be drawn onto the canvas because the view is essentially completely folded.
			 */
			if (mIsHorizontal) {
		...
		/*
		 * The shadows on the folds are split into two parts: Solid shadows and gradients. Every other fold has a solid shadow which overlays the whole fold. Similarly, the folds in between these alternating folds also have an overlaying shadow. However, it is a gradient that
		 * takes up part of the fold as opposed to a solid shadow overlaying the whole fold.
		 */
	...
	protected void dispatchDraw(Canvas canvas) {
		/**
		 * If prepareFold has not been called or if preparation has not completed yet, then no custom drawing will take place so only need to invoke super's onDraw and return.
		 */
		...
		for (int x = 0; x < mNumberOfFolds; x++) {
			...
			/*
			 * Concatenates the canvas with the transformation matrix for the the segment of the view corresponding to the actual image being displayed.
			 */
			...
			if (Util.IS_JBMR2) {
				...
				/*
				 * The same transformation matrix is used for both the shadow and the image segment. The canvas is clipped to account for the size of each fold and is translated so they are drawn in the right place. The shadow is then drawn on top of the different folds using the
				 * sametransformation matrix.
				 */
